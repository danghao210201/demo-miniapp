...
import data from "@/components/mock/dacsan.json";

const { Title } = Text;

/**
 * Trang chi tiết Đặc sản (dựa theo index trong danh sách dacsan.json)
 * - Đọc tham số :index từ URL để lấy item tương ứng trong mảng dữ liệu
 * - Hiển thị ảnh cover, tên, địa chỉ, giờ mở cửa, lịch hoạt động, hotline và gallery ảnh
 * - Cung cấp 2 hành động: Mở vị trí trên bản đồ, mở website (tận dụng LinkVR360 nếu có)
 */
export default function DacSanDetailPage() {
  // Lấy chỉ số từ URL và đảm bảo hợp lệ theo độ dài mảng dữ liệu
  const params = useParams();
  const index = useMemo(() => {
    const raw = params.index;
    const i = raw ? parseInt(raw, 10) : 0;
    return Number.isFinite(i) && i >= 0 && i < (data as any[]).length ? i : 0;
  }, [params.index]);

  const item: any = (data as any[])[index];

  const cover = (item?.HinhAnh?.[0] as string) ||
    "https://via.placeholder.com/800x450/e5e7eb/6b7280?text=No+Image";

  /**
   * Mở bản đồ dựa trên toạ độ ViTriMap (lat,lng) hoặc tìm kiếm theo địa chỉ
   */
  ...
  /**
   * Mở website/VR360 nếu có
   */
  const handleOpenWebsite = async () => {
    const url = item?.LinkVR360 || ""; // Tái sử dụng trường LinkVR360 làm Website nếu có
    if (!url) return;
    try {
      await openWebview({ url, config: { style: "normal" } });
    } catch (e) {
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className={`h-10 rounded-full font-medium flex items-center justify-center gap-2 active:scale-95 ${
                  item?.LinkVR360 ? "bg-primary text-white" : "bg-gray-100 text-gray-400"
                }`}
              >
                {/* Globe icon */}
                <svg
        ...
        {item?.HinhAnh?.length ? (
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              {item.HinhAnh.map((img: string, i: number) => (
